"""Example config for Seje_1.4.1957-30.9.1957-primer.pdf.
...
from doc2tei.parser import ParseResult
...
def session_date(chunk: PDFChunk) -> bool:
    return (
        ...
        and bool(
            re.fullmatch(r"\(\s*\d{1,2}\.\s+[^()]*?19\d{2}\.?\s*\)", line_text(chunk))
        )
    )
...
def speaker_parts(chunk: PDFChunk) -> tuple[str, str] | None:
    if not is_line_start(chunk) or not is_body_line(chunk) or is_front_matter(chunk):
        return None
...
def line_filter(record: LineRecord, _page: PDFPageContext) -> bool:
